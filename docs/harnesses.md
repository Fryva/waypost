...
Waypost renders one set of agent roles into whatever the harness in front of you
...
```bash
waypost harnesses                       # what is known, where roles land, how to invoke
waypost agents install --harness cursor # render into one
waypost agents install                  # …or into whatever this project uses
```
...
|----|---------|--------|---------------|-------|------------|
| `antigravity` | Google Antigravity | Google | `.agents/agents/waypost-<role>/agent.md` | subagent/rule | documented |
...
| `gemini` | Gemini CLI | Google | `.gemini/agents/waypost-<role>.md` | subagent/rule | documented |
| `grok` | Grok Build | xAI | `.grok/agents/waypost-<role>.md` | subagent/rule | documented |
| `kimi` | Kimi Code CLI | Moonshot AI | `.kimi-code/agents/waypost-<role>.md` | subagent/rule | documented |
| `lingma` | Tongyi Lingma (通义灵码) | Alibaba Cloud | `.lingma/rules/waypost-<role>.md` | prompt/rule | documented |
| `opencode` | OpenCode | SST | `.opencode/agents/waypost-<role>.md` | subagent/rule | documented |
| `pi` | Pi | Earendil | `— (no role files)` | routing block only | documented |
| `qm` | QM | Y Combinator | `— (no role files)` | routing block only | documented |
| `qwen` | Qwen Code | Alibaba | `.qwen/agents/waypost-<role>.md` | subagent/rule | documented |
| `roo` | Roo Code | Roo Code | `.roomodes` | custom mode | documented |
| `windsurf` | Windsurf | Cognition | `.windsurf/workflows/waypost-<role>.md` | prompt/rule | documented |
| `zcode` | ZCode (Z.ai / Zhipu) | Zhipu AI | `.zcode/agents/waypost-<role>.md` | subagent/rule | documented |
| `iflow` | iFlow CLI (心流) | iFlow | `.iflow/agents/waypost-<role>.md` | subagent/rule | inferred |
| `trae` | Trae (ByteDance) | ByteDance | `.trae/rules/waypost-<role>.md` | prompt/rule | inferred |
...
- **documented** — taken from the vendor's own documentation; the entry records
  the URL in its `docs` field (`waypost harnesses --json`), but it has not been run
  here.
...
the model side, which is what `Waypost-Provider` records:

```bash
export ANTHROPIC_BASE_URL=https://api.moonshot.ai/anthropic   # Kimi, for example
waypost agents install --harness claude    # install for the harness you actually run
waypost commit -m "…"                      # records Waypost-Provider: kimi
```

So there is nothing to install for a provider. What waypost does instead is
**record which one produced the work**: `waypost commit` detects the provider from
the endpoint override or a vendor key and writes an `Waypost-Provider` trailer, and
`waypost log --provider deepseek` reads it back. The same harness behaves very
...
`WAYPOST_PROVIDER` overrides the detection; with no evidence, nothing is recorded —
a guess in a permanent record is worse than a blank. Add your own provider as a
JSON file in `.waypost/harnesses/providers/`.
...
Drop a JSON file in `<project>/.waypost/harnesses/<id>.json`. A project entry with
...
- **`aggregate-json`** — every role is an entry in one shared JSON array, merged
  rather than overwritten: entries whose key starts with `waypost-` are ours,
...
- **Provenance.** Every generated file (or entry) carries a line naming the
  source role and a hash of the render. `waypost doctor` compares against what
  install would write *now*, so a changed role, model or adapter shows up as
  stale.
- **Nothing of yours is overwritten.** A file under the `waypost-` prefix without
