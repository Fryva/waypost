...
3. **Identify the kind**: parse the YAML frontmatter for `type:`. If missing, try to infer from the file path (`adr/` → adr, `epics/<id>/epic.md` → epic, `epics/<id>/stories/*` → story, `research/` → research, etc.). If still unknown, ask the user by asking the user which kind to apply.
...
8. **Ask the user by asking the user** what to do:
...
9. **On any apply path**, after each Edit (approved by the user), update the frontmatter:
