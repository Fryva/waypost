...
Instructions for any agent harness working on **Waypost**.

Waypost is a harness-agnostic fork of ProjectStore: one project engine (decisions,
specs, epics, stories, a kanban board — markdown in git), one CLI `waypost`, and one
...
Run `waypost` with no arguments. It answers the only two questions that matter: is
this project set up, and what does it need right now. If it is not set up,
`waypost setup` does the whole install in one command — bind a vault, scaffold it,
...
- **A deterministic doctor**: `waypost doctor` checks consistency with no AI.

## The CLI (instead of slash commands)

| Command | What it does |
|---------|--------------|
| `waypost setup` | the whole install in one command (`--dry-run` to look first) |
| `waypost next` | what needs doing now, ranked, with the command for each |
...
A role is defined once in `agents/<role>.md`; `waypost agents install` renders it
into the format of whichever harness the project uses. `waypost harnesses` lists
...
in `harnesses/`, and your own goes in `.waypost/harnesses/` (see
...
- `waypost-planner` — before implementing an epic or story: a plan grounded in how
  previous epics landed in the code (`code_refs`).
- `waypost-reviewer` — after the code, before commit or story-done: does the diff
  close the acceptance criteria?
- `waypost-librarian`, `waypost-archaeologist` — semantic vault curation, and recovering
...
- Roles are read-only by contract: edits are denied by the tool map or sandbox
  ...
  by the role prompt. They propose; writes go through the approval-gated `waypost`
  flow.
- Role models: `waypost agents model default <id>` applies to harnesses with a
  published tier naming (Claude Code); for the others use
  `waypost agents model harness:<name> <id>`. Re-run `waypost agents install` afterwards.
- A vendor's CLI and a vendor's models are different entries. Moonshot, Zhipu,
  ...
  `waypost agents show <role>` or through a subagent that harness can spawn.

## Working in several harnesses at once

One project may be driven by several sessions at the same time (ADR-0006):

- **Commit through `waypost commit -m "<what>" [--story <id>] [--all]`.** It
  reconciles the derived views, checks other sessions' claims, and writes the
  `Waypost-Harness` / `Waypost-Session` / `Waypost-Provider` / `Waypost-Story` trailers. One
  commit, one story.
- **`waypost log [--story|--harness|--provider|--session <v>]`** reads that back. A
...
- **Start of work**: any working command beats the heartbeat automatically; for a
  long session run `waypost watch` in the background. `waypost brief` and `waypost status`
  show who else is live and what they hold.
- **Before editing files someone else might touch**: `waypost lease <path…>`. A
  refusal names the device and harness holding it. `waypost lease release` when done;
  `waypost commit` refuses to write over a live foreign lease.
...
- **Never read derived views whole.** `graph.md` and the indexes grow linearly
  (~66 and ~51 tokens per artifact). Use `waypost graph --for <path>` and
  `waypost search "<text>" [--kind] [--limit]`: at 32 artifacts that is 44 and 91
...
- **Detail is behind a flag.** By default commands answer the question:
  `waypost brief` (`--full`), `waypost harnesses` (`--all`), `waypost agents list` (`-v`),
  `waypost draft` without `--write` (`--json`).
...
- **A role is the most expensive operation** ($1.62 for a critic pass on Opus, of
  ...
  every edit; `waypost agents model default sonnet` cuts it by ~60%.
...
- Environment: `WAYPOST_PROJECT_DIR`, `WAYPOST_HOME`, `WAYPOST_SESSION_ID`, `WAYPOST_HARNESS`,
  `WAYPOST_PROVIDER`, `WAYPOST_NO_BEAT`.
...
- **Code** (`.mjs`, `bin/waypost`) is pure node with no external dependencies;
...
- A new role is a file in `agents/` with neutral frontmatter, not a change to the
  adapters.
- A new harness is a JSON file in `harnesses/` (or `.waypost/harnesses/` in a
